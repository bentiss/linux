...
/**
 * hid_exists_dquirk: find any dynamic quirks for a USB HID device
...
/**
 * hid_quirks_init: apply USB HID quirks specified at module load time
 */
...
/**
 * hid_gets_squirk: return any static quirks for a USB HID device
 * @hdev: the HID device to match
 *
 * Description:
...
static unsigned long hid_gets_squirk(const struct hid_device *hdev)
{
	...
	for (n = 0; hid_ignore_list[n].bus; n++)
		if (hid_match_one_id(hdev, &hid_ignore_list[n]))
			quirks |= HID_QUIRK_IGNORE;

	for (n = 0; hid_have_special_driver[n].bus; n++)
		if (hid_match_one_id(hdev, &hid_have_special_driver[n]))
			quirks |= HID_QUIRK_HAVE_SPECIAL_DRIVER;

	for (n = 0; hid_quirks[n].bus; n++)
		if (hid_match_one_id(hdev, &hid_quirks[n]))
			quirks |= hid_quirks[n].driver_data;
...
unsigned long hid_lookup_quirk(const struct hid_device *hdev)
{
	...
	if (hdev->vendor == USB_VENDOR_ID_NCR &&
