...
/*
 * reimplementing part of __hid_request() here so as to be able
 * to communicate with the device even if hid_hw_start() has not been called.
 */
static int wacom_hid_hw_request(struct hid_device *hdev,
				struct hid_report *report, int reqtype)
{
	...
	if (reqtype == HID_REQ_GET_REPORT) {
		retval = wacom_get_report(hdev, report->type, buf, len, 1);
	} else {
		hid_output_report(report, buf);
		retval = wacom_set_report(hdev, report->type, buf, len, 1);
	}
...
{
	if (features->type == HID_GENERIC)
		wacom_wac_post_parse_hid(hdev, features);
}
...
static int wacom_led_control(struct wacom *wacom)
{
	...
	if (wacom->wacom_wac.features.type == HID_GENERIC) {
		...
		wacom_hid_hw_request(wacom->hdev, report, HID_REQ_SET_REPORT);
...
static int wacom_leds_read(struct wacom *wacom)
{
	...
	wacom_hid_hw_request(wacom->hdev, report, HID_REQ_GET_REPORT);
...
static void wacom_destroy_leds(struct wacom *wacom)
{
	...
	case HID_GENERIC:
		sysfs_remove_group(&wacom->hdev->dev.kobj,
				   &hid_generic_led_attr_group);
		hid_data->led_count = 0;
		hid_data->group_led_count = 0;
		hid_data->group_led_brightness_count = 0;
		for (i = 0; i < ARRAY_SIZE(hid_data->led_control); i++) {
			hid_data->led_control[i] = NULL;
		}
		break;
	}
}
...
static void wacom_fw_work(struct work_struct *fw_work)
{
	...
	wacom_query_tablet_data(hdev, &wacom->wacom_wac.features);
