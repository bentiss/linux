...
enum led_brightness wacom_leds_brightness_get(struct wacom_led *led)
{
	...
	return led->llv ? LED_FULL : LED_OFF;
}

static enum led_brightness __wacom_leds_brightness_get(struct led_classdev *cdev)
{
...
static int wacom_leds_brightness_set(struct led_classdev *cdev,
...
static void wacom_leds_readonly_brightness_set(struct led_classdev *cdev,
					       enum led_brightness brightness)
{
}
				  ...
				  unsigned int id, bool read_only)
{
	...
	led->cdev.brightness_get = __wacom_leds_brightness_get;
	if (!read_only) {
		led->cdev.brightness_set_blocking = wacom_leds_brightness_set;
		led->cdev.default_trigger = led->cdev.name;
	} else {
		led->cdev.brightness_set = wacom_leds_readonly_brightness_set;
	}
...
{
	struct wacom_remote *remote =remote = wacom->remote;
	u32 serial = remote->serial[index];
	int i;

	if (remote->remote_group[index].name)
		devres_release_group(&wacom->hdev->dev, &remote->input[index]);
				   ...
				   unsigned int index)
{
	...
	if (!devres_open_group(dev, &remote->input[index], GFP_KERNEL))
		return -ENOMEM;
	...
	devres_close_group(dev, &remote->input[index]);
	return 0;

fail:
	devres_release_group(dev, &remote->input[index]);
