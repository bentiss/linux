...
int elantech_query_info(struct psmouse *psmouse, struct elantech_device_info *info)
{
	unsigned char param[3];
	...
	if (info->hw_version != 1) {
		...
		psmouse_info(psmouse,
			     ...
			     info->samples[0], info->samples[1], info->samples[2]);
	}
...
static int elantech_smbus = IS_ENABLED(CONFIG_ELAN_I2C) ?
...
int elantech_init(struct psmouse *psmouse)
{
	...
	case ETP_BUS_PS2_SMB_ALERT:
		psmouse_info(psmouse, "Ignoring SMBus provider through alert protocol.\n");
		break;
	...
	case ETP_BUS_PS2_SMB_HST_NTFY:
		...
		break;
	default:
		psmouse_info(psmouse, "Ignoring SMBus bus provider %d.\n",
			     info.bus);
	}
